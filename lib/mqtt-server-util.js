...
function getMqttConfig() {
  const clientId = `smart-doorbell-server-${Math.random()
    .toString(16)
    .substring(2, 8)}`;
...
function setupPersistentEventHandlers(clientInstance) {
  const handleClose = () => {
  ...
  const handleMessageWrapper = (topic, message) => {
...
export async function connectMqttWithHandlers() {
    ...
    if (needsBrokerSubscription) {
      ...
      try {
        ...
        if (granted && granted.length > 0) {
          console.log(
            `[MQTT Server Util] 👍 Subscribed to ${
              granted[0]?.topic ?? topic
            } with QoS ${granted[0]?.qos ?? "unknown"}`
          );
        } else {
          console.warn(
            `[MQTT Server Util] ⚠️ Subscription to ${topic} ack'd by broker, but grants array is empty or unclear.`
          );
        }
        return true;
      } catch (err) {
...
export async function unsubscribeFromTopic(topic, callback) {
  ...
  try {
    ...
    if (callbacks.size === 0) {
      ...
      if (currentClient && currentClient.connected) {
        ...
        console.warn(
          `[MQTT Server Util] Client not connected, cannot unsubscribe ${topic} from broker now.`
        );
        return true;
      }
    } else {
...
export function disconnectMqtt() {
  if (client) {
    const clientUrl =
      client.options?.host || client.options?.hostname || "unknown URL";
    console.log(
    ...
    console.log("[MQTT Server Util] No active MQTTS client to disconnect.");
    return false;
  }
}
