    ...
    if (needsBrokerSubscription) {
      ...
      try {
        ...
        if (granted && granted.length > 0) {
          console.log(
            `[MQTT Server Util] 👍 Subscribed to ${granted[0]?.topic ?? topic} with QoS ${granted[0]?.qos ?? 'unknown'}`
          );
        } else {
          console.warn(
  ...
  try {
    ...
    if (callbacks.size === 0) {
      ...
      if (currentClient && currentClient.connected) {
        ...
        console.warn(`[MQTT Server Util] Client not connected, cannot unsubscribe ${topic} from broker now.`);
        return true;
      }
    } else {
      return true;
...
export function disconnectMqtt() {
  if (client) {
    const clientUrl = client.options?.host || client.options?.hostname || "unknown URL";
    console.log(
